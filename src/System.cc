...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const bool bReuse, const int pnp_version):mSensor(sensor),mbReset(false),mbActivateLocalizationMode(bReuse),
        mbDeactivateLocalizationMode(false), pnp_version(pnp_version)
{
	...
	if (bReuse)
	{
		LoadMap("/home/george/Documents/ORB_SLAM2/Slam_latest_Map.bin");
    ...
    if (pnp_version == 0)
	{
		window_name = "OPENCV PNP";
	}
    else if (pnp_version == 1)
	{
		window_name = "NEW PNP";
	}
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const int pnp_version)
{
    ...
    return (mpTracker->GrabImageMonocular(im,timestamp, pnp_version));
}
...
void System::Shutdown()
{
    ...
    pangolin::BindToContext("ORB-SLAM2: Map Viewer " + window_name);
}
