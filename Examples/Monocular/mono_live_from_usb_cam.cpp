...
int main(int argc, char **argv)
{
	...
	ORB_SLAM2::System SLAM(vocabularyPath, cameraSettingsPath,ORB_SLAM2::System::MONOCULAR,true, true);
...
//			im = cv::imread(vstrImageFilenames[ni],CV_LOAD_IMAGE_UNCHANGED);
		...
		if(im.empty())
		{
			cerr << "Blank frame grabbed, image index: " << ni << endl;
//			return 1;
			continue;
		}
...
)
{
	...
	while(!fTimes.eof())
	{
		...
		if(!s.empty())
		{
			...
			result = split(result[1], '\r');

			vstrImages.push_back(strImagePath + "/" + result[0]);
