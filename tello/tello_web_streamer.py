...
app = Flask(__name__)
resource_path = os.path.join(app.root_path, 'templates')

global_frame = None
connect_tello_frame = cv2.imread(os.path.join(resource_path, 'connect_tello.jpeg'))
lock = threading.Lock()
should_stop = False
...
def write_image_to_pipe():
    ...
    while not should_stop:
        fd = os.open(FIFO_Images, os.O_RDWR)    # non-blocking
        ...
        frame = cv2.resize(frame, (int(960), int(720)))
    ...
    while True:
        ...
        with lock:
            ...
            if global_frame is None:
                # continue
                global_frame = connect_tello_frame
...
if __name__ == '__main__':
    ...
    try:
        # start a thread that will perform
        t = threading.Thread(target=tello_thread)  # , args=(args["frame_count"],))
        t.daemon = True
        t.start()

        # start piping thread
        pipe_thread = threading.Thread(target=write_image_to_pipe, name="pipe_thread")
